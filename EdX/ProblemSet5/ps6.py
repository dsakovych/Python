...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        if shift >= 0 or shift < 26:
            letters = [item for item in string.ascii_lowercase + string.ascii_uppercase]
            copy_letters = letters[:]
            letters.reverse()
            letters = letters[-shift:] + letters[:len(letters) - shift]
            letters.reverse()
            return dict(zip(copy_letters, letters))
